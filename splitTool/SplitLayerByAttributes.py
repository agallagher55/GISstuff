...
Author:    Dan.Patterson@carleton.ca
           gallaga.halifax.ca - csv exports
Created:   2005-06-23
Modified:  2019-06-26  (last change date)
Purpose:
...
csvExport = sys.argv[5]
shapefileExport = sys.argv[6]
...
for aVal in valueList:
  ...
  try:
    ...
    tempLayer = arcpy.MakeFeatureLayer_management(input_FC,  "TempLayer",  whereClause)

    if shapefileExport == 'true':
    ...
    # WRITE TO .CSV
    if csvExport == 'true':
      arcpy.AddMessage("\tWriting {} to .csv...".format(os.path.basename(outShapeFile)).replace(".shp", ""))
      outputCsv = outShapeFile.replace(".shp", ".csv")

      with arcpy.da.SearchCursor(tempLayer, "*") as cursor:
        for row in cursor:
          with open(outputCsv, 'wb') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([f.name for f in field_list])
            writer.writerow(row)
